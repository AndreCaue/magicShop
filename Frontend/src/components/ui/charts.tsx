    ...
    return (
      ...
      >
        {payload.map((item) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          const valuePercent = `${(((item.payload as { percent?: number }).percent ?? 0) * 100).toFixed(1)}%`;

          return (
            <div
              key={item.value}
              className={cn(
                "flex items-center gap-1 rounded-md bg-[#E2DAFA] px-3 py-1 [&>svg]:h-2 [&>svg]:w-2 [&>svg]:text-muted-foreground",
              )}
            >
