...
type TInverseMatrixDeck = {
  children: React.JSX.Element;
};

export default function InverseMatrixDeck({ children }: TInverseMatrixDeck) {
  return (
    <div className="relative inset-0 bg-black overflow-hidden w-screen h-screen">
      {deck.map((card, i) => {
        const chars = [...card];
        ...
        return (
          <motion.div
            key={card}
            className="absolute"
            style={{ left: `${left}%`, bottom: -100, willChange: "transform" }}
            initial={{ y: 100 }}
      ...
      <div className="pointer-events-none fixed inset-0 bg-gradient-to-t from-transparent via-green-950/30 to-black" />
