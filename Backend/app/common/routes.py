...
from .schemas import ShippingPresetDropdown, RefundReasonResponse, REFUND_REASON_METADATA
...
async def get_category_dropdown(db: Session = Depends(get_db), _: User = Depends(require_master_full_access)):
    category = db.query(Category.id, Category.name).order_by(
...
@router.get("/refund-reasons", response_model=List[RefundReasonResponse])
def get_refund_reasons():
    return [
        ...
        for reason, metadata in REFUND_REASON_METADATA.items()
    ]
